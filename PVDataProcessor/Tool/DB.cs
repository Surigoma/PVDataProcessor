...
namespace PVDataProcessor.Tool
{
    public class DB
    {
                    ...
                    else
                    {
                        ...
                        for (int ii = 0; ii < buff.LongLength; ii++)
                        {
                            ...
                            if (sd.SamplingDate.Hour - h < 0)
                            {
                                ...
                                sd.SamplingDate = sd.SamplingDate.AddDays(1);
                            }
        ...
        public void LoadDay(string FilePath)
        {
            if (Path != FilePath)
            {
                Path = FilePath;
                ...
                while ((i = sr.ReadLine()) != null)
                {
                    ...
                    else
                    {
                        ...
                        for (int ii = 0; ii < buff.LongLength - 1; ii++)
                        {
                            string[] c = ListPath[ii].Split(' ');
                            int PH = 0, PPA = 0, B = 0;
                            if (c.LongLength > 1)
                            ...
                            {
                                case "Time":
                                    sd.SamplingDate = DateTime.Parse(buff[ii]);
                                    break;
                                case "Watts":
                                    sd.Data[PPA].PHData[PH].Watts = double.Parse(buff[ii]);
                                    break;
